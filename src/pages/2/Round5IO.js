...
const handleUPRecommendationClick = () => {
    ...
    if (wantUPR) {
      ...
      sendUserPreferencesToDB(preferences, 5); // 수정 5
...
const handleInvestButtonClick = async () => {
  ...
  if (confirmation) {
      ...
      try {
          ...
          window.location.href = '/2/result-five'; // 수정 8
      } catch (error) {
...
return (
    <div className='exp-container'>
      <h2>Round 5</h2> {/* 수정 9 */}
        ...
        <p>투자금 (Beginning Balance): {profits?.profit_4 ?? 'Loading...'}</p> {/* 수정 10 */}
          <div className="timer">
