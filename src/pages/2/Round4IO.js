...
function RoundFour() { // 라운드 수정 1
  ...
  useEffect(() => {
    ...
    if (timerActive && timer > 0) {
      ...
      window.location.href = '/2/result-four'; // 라운드 수정 3 // 사이트 수정 1
    }
...
const handleInvestButtonClick = async () => {
  ...
  if (confirmation) {
      ...
      try {
          ...
          window.location.href = '/2/result-four'; // 라운드 수정 7 // 사이트 수정 2
      } catch (error) {
...
return (
    <div className='exp-container'>
      <h2>Round 4</h2> {/* 라운드 수정 8 */}
        <div style={containerStyles(false)}>
        <p>투자금 (Beginning Balance): {profits?.profit_3 ?? 'Loading...'}</p> {/* 라운드 수정 9 */}
          <div className="timer">
