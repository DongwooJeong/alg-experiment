...
function RoundTen() { // 수정 1
  ...
  useEffect(() => {
    ...
    if (timerActive && timer > 0) {
      ...
      window.location.href = '/2/result-one'; // 페이지 이동
    }
...
const handleUPRecommendationClick = () => {
    ...
    if (wantUPR) {
      ...
      sendUserPreferencesToDB(preferences, 10); // 수정 5
...
const handleInvestButtonClick = async () => {
  ...
  if (confirmation) {
      ...
      try {
          ...
          window.location.href = '/2/result-ten'; // 수정 8
      } catch (error) {
...
return (
    <div className='exp-container'>
      <h2>Round 10</h2> {/* 수정 9 */}
        ...
        <p>투자금 (Beginning Balance): {profits?.profit_9 ?? 'Loading...'}</p> {/* 수정 10 */}
          <div className="timer">
