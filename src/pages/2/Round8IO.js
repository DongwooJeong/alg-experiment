...
function Round8IO() { // 수정 1
  ...
  useEffect(() => {
    ...
    if (timerActive && timer > 0) {
      ...
      window.location.href = '/2/result-eight'; // 수정 
    }
...
const handleInvestButtonClick = async () => {
  ...
  if (confirmation) {
      ...
      try {
          ...
          window.location.href = '/2/result-eight'; // 수정 4
      } catch (error) {
...
return (
    <div className='exp-container'>
      <h2>Round {round}</h2>
        ...
        <p>초기 투자금 (Beginning Balance): {profits?.profit_7 ?? 'Loading...'}</p> {/* 수정 5 */}
          <div className="timer">
