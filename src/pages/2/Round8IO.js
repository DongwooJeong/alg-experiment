...
const handleUPRecommendationClick = () => {
    ...
    if (wantUPR) {
      ...
      sendUserPreferencesToDB(preferences, 8); // 수정 5
...
const handleInvestButtonClick = async () => {
  ...
  if (confirmation) {
      ...
      try {
          ...
          window.location.href = '/2/result-eight'; // 수정 8
      } catch (error) {
...
return (
    <div className='exp-container'>
      <h2>Round 8</h2> {/* 수정 9 */}
        ...
        <p>투자금 (Beginning Balance): {profits?.profit_7 ?? 'Loading...'}</p> {/* 수정 10 */}
          <div className="timer">
