...
function ResultSeven() { // 수정 1
    ...
    const finalProfit = useMemo(() => {
        ...
        // 수정 3
        let startingCapital = userProfits.profit_6;
    ...
    return (
        <div className='result-container'>
            ...
            <div>
                ...
                <p>7라운드 후 투자자의 누적 수익 (Cumulative Profit after Round 7): {finalProfit}</p> {/* 수정 6 */}
            </div>
            <div class='buttonCenter'>
            <button className="btn-green" onClick={() => {
                saveUserProfit(finalProfit).then(() => {
                    window.location.href = '/2/round-eight'; // {/* 수정 7 */}
                })
                .catch(error => {
                    // 에러 처리 로직
